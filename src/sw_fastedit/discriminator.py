...
class Discriminator(nn.Module):
     def __init__(self, num_in_channels, ndf = 64, pool_size=(4, 4, 4)):
          ...
          self.conv1 = Convolution(spatial_dims=3, in_channels=num_in_channels, out_channels=ndf, act=('leakyrelu'), kernel_size=4, strides=2, padding=1)
          self.conv2 = Convolution(spatial_dims=3, in_channels=ndf, out_channels=ndf*2, act=('leakyrelu'), kernel_size=4, strides=2, padding=1)
          self.conv3 = Convolution(spatial_dims=3, in_channels=ndf*2, out_channels=ndf*4, act=('leakyrelu'), kernel_size=4, strides=2, padding=1)
          self.conv4 = Convolution(spatial_dims=3, in_channels=ndf*4, out_channels=ndf*8, act=('leakyrelu'), kernel_size=4, strides=2, padding=1)
