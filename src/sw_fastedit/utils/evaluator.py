...
__all__ = ["Evaluator", "SupervisedEvaluatorEp", "SupervisedEvaluatorDynUnet", "SupervisedEvaluatorDynUnet","SupervisedEvaluatorDextr", "SupervisedEvaluatorPada", "SupervisedEvaluatorUgda"]
...
class SupervisedEvaluatorEp(Evaluator):
    ...
    def _iteration(self, engine: SupervisedEvaluatorEp, batchdata: dict[str, torch.Tensor]) -> dict:
        """
...
class SupervisedEvaluatorDynUnet(Evaluator):
    ...
    def _iteration(self, engine: SupervisedEvaluatorDynUnet, batchdata: dict[str, torch.Tensor]) -> dict:
        """
        callback function for the Supervised Evaluation processing logic of 1 iteration in Ignite Engine.
        Return below items in a dictionary:
            - IMAGE: image Tensor data for model input, already moved to device.
        ...
        logger.info(f'Dice Helper: {dice_helper}')
        engine.fire_event(IterationEvents.MODEL_COMPLETED)
        
        return engine.state.output
    
class SupervisedEvaluatorDextr(Evaluator):
    """
    Standard supervised evaluation method with image and label(optional), inherits from evaluator and Workflow.
    ...
    def _iteration(self, engine: SupervisedEvaluatorDextr, batchdata: dict[str, torch.Tensor]) -> dict:
        """
        callback function for the Supervised Evaluation processing logic of 1 iteration in Ignite Engine.
        Return below items in a dictionary:
            - IMAGE: image Tensor data for model input, already moved to device.
        ...
        engine.state.output[Keys.SEG_EP] = torch.cat((inputs, target_ep), dim=1) 
        
        # execute forward computation
        with engine.mode(engine.network):
            if engine.amp:
                with torch.cuda.amp.autocast(**engine.amp_kwargs):
                    engine.state.output[Keys.PRED_SEG] = engine.inferer(engine.state.output[Keys.SEG_EP], engine.network[1], *args, **kwargs)
            else:
                engine.state.output[Keys.PRED_SEG] = engine.inferer(engine.state.output[Keys.SEG_EP], engine.network[1], *args, **kwargs)
...
class SupervisedEvaluatorPada(Evaluator):
    """
    Standard supervised evaluation method with image and label(optional), inherits from evaluator and Workflow.
    ...
    def _iteration(self, engine: SupervisedEvaluatorPada, batchdata: dict[str, torch.Tensor]) -> dict:
        """
        callback function for the Supervised Evaluation processing logic of 1 iteration in Ignite Engine.
        Return below items in a dictionary:
            - IMAGE: image Tensor data for model input, already moved to device.
        ...
        def forward_pass():
            if self.args.extreme_points:
                engine.state.output[Keys.SEG_EP] = torch.cat((inputs, target_ep), dim=1)
                engine.state.output[Keys.PRED_SEG] = engine.inferer(engine.state.output[Keys.SEG_EP], engine.network[1], *args, **kwargs)
                engine.state.output[GanKeys.GPRED] = engine.inferer(engine.state.output[Keys.PRED_SEG], engine.network[2], *args, **kwargs)
            else:
                engine.state.output[Keys.PRED_SEG] = engine.inferer(inputs, engine.network[1], *args, **kwargs)
                engine.state.output[GanKeys.GPRED] = engine.inferer(engine.state.output[Keys.PRED_SEG], engine.network[2], *args, **kwargs)
        ...
        logger.info(f'Dice Metric: {dice_helper[0].item()} dis: {engine.state.output[GanKeys.GPRED].item()}')
...
class SupervisedEvaluatorDualDynUnet(Evaluator):
    """
    Standard supervised evaluation method with image and label(optional), inherits from evaluator and Workflow.
    ...
    def _iteration(self, engine: SupervisedEvaluatorDualDynUnet, batchdata: dict[str, torch.Tensor]) -> dict:
        """
        callback function for the Supervised Evaluation processing logic of 1 iteration in Ignite Engine.
        Return below items in a dictionary:
            - IMAGE: image Tensor data for model input, already moved to device.
        ...
        def forward_pass():  
            image_ep = torch.cat((inputs, target_ep), dim=1)
            engine.state.output[Keys.PRED_SEG] = engine.inferer(image_ep, engine.network[1], *args, **kwargs)

            #just for examening ep
            engine.state.output[Keys.PRED_EP] = engine.inferer(inputs, engine.network[0], *args, **kwargs)
            pred_ep_processed = torch.where(engine.state.output[Keys.PRED_EP] > 0.1, engine.state.output[Keys.PRED_EP], torch.tensor(0.0, device=engine.state.device)) 
            engine.state.output['pred_ep_processed'] = pred_ep_processed
        ...
        #evaluation segmentation
        mse_metric = self.mse_metric(y_pred=engine.state.output[Keys.PRED_EP], y=target_ep)
        dice_helper = self.dice_helper(y_pred=engine.state.output[Keys.PRED_SEG], y=target_seg)


        logger.info(f'Dice Metric: {dice_helper[0].item()} mse: {mse_metric.item()}')
...
class SupervisedEvaluatorUgda(Evaluator):
    """
    Standard supervised evaluation method with image and label(optional), inherits from evaluator and Workflow.
    ...
    def _iteration(self, engine: SupervisedEvaluatorUgda, batchdata: dict[str, torch.Tensor]) -> dict:
        """
        callback function for the Supervised Evaluation processing logic of 1 iteration in Ignite Engine.
        Return below items in a dictionary:
            - IMAGE: image Tensor data for model input, already moved to device.
        ...
        def forward_pass():
            engine.state.output[Keys.SEG_EP] = torch.cat((inputs, target_ep), dim=1)
            engine.state.output[Keys.PRED_SEG] = engine.inferer(engine.state.output[Keys.SEG_EP], engine.network[1], *args, **kwargs)
            pred_seg_ep = torch.cat((engine.state.output[Keys.PRED_SEG], target_ep), dim=1)
            engine.state.output[GanKeys.GPRED] = engine.inferer(pred_seg_ep, engine.network[2], *args, **kwargs)
